<?php
class steed_element_div_style_footer_warp{
	...
	function __construct(){
		$this->customize_prefix = 'footer_warp_style_';
		$this->customize_panel = 'site_Footer';
		$this->customize_section = 'steed_footer_warp_style';
		$this->style_class = '#site-footer';
		
		add_filter('steed_custom_css', array($this, 'css'));
		add_action( 'customize_register', array($this, 'customize') );
		
		add_filter('steed_footer_warp_color_mood', array($this, 'color_mood'));
	}	
	
	function color_mood(){
		echo ' color-'.esc_attr(steed_theme_mod($this->customize_prefix.'color_mood'));
	}
	...
	function customize($wp_customize){
		...
		$wp_customize->add_section( $this->customize_section , array(
			'title'      => __( 'Footer Style and Background', 'steed' ),
			'priority'   => 30,
		...
		$wp_customize->add_control(new steed_Customize_Control_heading($wp_customize, $uid, array(
			...
			'settings'   => $uid,
			'description' => 'Footer Warp Background style',
			'priority'   => 7,
		)));
...
new steed_element_div_style_footer_warp;