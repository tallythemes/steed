<?php
class steed_element_div_style_branding{
	...
	function __construct(){
		$this->customize_prefix = 'branding_style_';
		$this->customize_panel = 'site_header';
		$this->customize_section = 'steed_branding_style';
		$this->style_class = '.braning';
		
		add_filter('steed_custom_css', array($this, 'css'));
		add_action( 'customize_register', array($this, 'customize') );
		
		add_filter('steed_branding_color_mood', array($this, 'color_mood'));
	}	
	
	function color_mood(){
		echo ' color_'.esc_attr(steed_theme_mod($this->customize_prefix.'color_mood'));
	}
	...
	function customize($wp_customize){
		...
		$wp_customize->add_control(new steed_Customize_Control_heading($wp_customize, $uid, array(
			...
			'settings'   => $uid,
			'description' => 'Branding Area Background style',
			'priority'   => 7,
		)));
...
new steed_element_div_style_branding;